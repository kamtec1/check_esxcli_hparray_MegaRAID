#!/usr/bin/env python3
"""
Nagios plugin for MegaRAID RAID monitoring on physical servers via storcli64
Version: v1.7
"""
...
class LocalStorCLIRunner:
    """Executes storcli commands locally"""

    def __init__(self, timeout: int = 60, controller: str = "0"):
        self.timeout = timeout
        self.controller = controller

    def run(self, cmd: str) -> Tuple[str, int]:
        """Run a storcli command and return output and exit code"""
        try:
            full_cmd = [STORCLI_PATH] + cmd.split()
...
class RaidChecker:
    ...
    def __init__(self, runner: LocalStorCLIRunner, vd_num: str = None):
        self.runner = runner
        self.vd_num = vd_num
    ...
    def check_battery(self):
        ...
        if not self.status.battery_status:
            ctrl_output, _ = self.runner.run(f"{ctrl} show all")
    ...
    def evaluate_status(self) -> Tuple[int, str]:
        ...
        return STATE_OK, f"{msg}{perf_suffix}{long_out}"
...
"""
    ...
    if not os.path.exists(STORCLI_PATH):
        print(f"RAID UNKNOWN - storcli64 not found at {STORCLI_PATH}")
        sys.exit(STATE_UNKNOWN)
    if os.path.exists('/tmp/NO_CHECK'):
        print("RAID OK - Check skipped (maintenance mode)")
        sys.exit(STATE_OK)

    runner = LocalStorCLIRunner(
